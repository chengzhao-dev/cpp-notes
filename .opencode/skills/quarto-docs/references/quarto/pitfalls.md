...
- **规避**：**项目/目录/文件名一律使用纯 ASCII 字符**；
  ...
  可用 `.opencode/skills/quarto-docs/scripts/check_ascii_names.py` 校验整个仓库。
...
- **根因**：本仓库约定每个 `##`/`###` 前写一条 `---` 作章节分隔线（opencode 风）。若 `---` 紧接前一段、中间**没有空行**，Markdown 的 setext 语法会把「前一段文本 + `---`」当成一个二级标题（h2）。
