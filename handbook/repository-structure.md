...
| `scripts/` | 仓库级 Python；知识库管道（`kb_util`/`chunker`/`indexer`/`retriever`/`evaluate`/`eval_set`/`health_check`/`scale_benchmark`/`test_conflict_detection`）与仓库脚本 |
...
| `scripts/`（根） | 知识库管道：分块 → 双层索引与图谱 → 五阶段检索 → 评测 → 体检 → 规模基准，入口统一走 `run.py kb-*` |
