...
Quarto 的 `body-width` 是上限声明，不是保证值。`sidebar-width + body-width + margin-width + 2 × gutter-width` 超过目标视口时，浏览器按比例压缩正文列，作者在配置里看到的仍是自己写的数字，所以改栅格前先做减法。本项目当前三栏声明合计为 1280px（`240 + 800 + 208 + 32`），正好覆盖目标桌面视口。正文上限为 800px。`margin-width` 保留 208px，给右侧目录折行余量，同时让中栏适合中文连续阅读。
