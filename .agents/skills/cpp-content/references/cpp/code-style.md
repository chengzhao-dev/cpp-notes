...
代码块只承担一个主要学习目标。多行原理说明移到正文，注释放在被说明代码的上一行，不使用行尾长注释；讲解同一概念的后续示例应减少重复注释。C++ 使用 `cpp` 围栏和 `//` 注释；Linux/WSL 命令和 Shell 脚本使用 `bash` 围栏，只放可敲命令，不加 `$ ` 提示符与 `#` 注释；CMake 使用 `cmake` 围栏、`#` 注释。命令输出使用紧随其后的 `text` 块，细则见 `quarto-docs` 的 `terminal-validation.md`。各代码块内部的逻辑块之间留一个空行。
