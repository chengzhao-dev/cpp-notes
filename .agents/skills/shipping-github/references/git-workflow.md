...
通用 git 操作规范。默认环境：Windows + PowerShell。**默认不提交、不推送、不建 PR：只在用户明确要求时才执行，范围只覆盖用户指定的路径组与目标分支。**
...
git init -b main
git add --renormalize .
git status --porcelain        # 核验：无 node_modules/_book/.quarto/runtime.json/temp 条目
git commit -m "初始提交：…"
...
`check`、`verify`、`build` 和 `render` 只证明本地改动可用，不是上传完成。用户明确要求提交并推送时，必须继续完成显式暂存、提交、`git fetch`、必要 rebase、推送、远端 SHA 和 CI/Pages 核对；其中任一项失败都不得报告任务完成。
...
README 面向 GitHub 阅读，保留徽章、图片和外部链接的标准 Markdown 写法并为图片提供替代文字。Skill、`.agents/knowledge/` 和 `AGENTS.md` 以纯文本、表格、编号步骤和代码块为主，默认不放 Mermaid 或图片。改完 Markdown 跑统一文档检查，避免把 GitHub 专用语法误判为 Quarto 错误。
