...
```powershell
& .agents/skills/governing-agents/scripts/run.ps1 kb-search "GitHub Pages 部署方式与产物分支" --domain shipping-github
```
...
2. `quarto render` → `python3 .agents/skills/maintaining-python/scripts/render/defer_mermaid.py`（本地 render 不自动跑，CI 必须执行）。
...
- [ ] 示例经 `.agents/skills/writing-cpp/scripts/verify_examples.py` 编译校验
