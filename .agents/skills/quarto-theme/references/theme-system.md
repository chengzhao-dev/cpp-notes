...
- Quarto `grid.body-width` 是正文列的上限。sidebar、body、margin 与两条 gutter 的合计超过视口时，浏览器会压缩正文列。当前取值只在 `_quarto.yml` 维护，改这三个值前先做加法核对目标视口，再回看右侧目录是否折行。
- `--content-width` 只限制正文容器的最大宽度，普通段落、列表、代码块、表格和 callout 都使用实际正文列宽。主题 CSS 不得用 `75ch` 等第二层限制覆盖，窄屏由父容器缩放，内容保持 `max-width: 100%` 并避免横向溢出。
- 首页标题描述等明确的短导语可以单独使用 `ch` 控制长度。
- ≤991.98px 时 Quarto 把左侧栏从常驻栏改成覆盖正文的浮层，该状态必须有不透明底色：透明底会让下方正文透上来叠在侧栏文字上。桌面端保持透明，靠页面底色融为一体；浮层另加一条右发丝线把两层分开。成因与验收依据见标识 `cpp-quarto-sidebar-overlay-v1` 的知识文件。
...
- **界面文案中文口径**：Quarto 只本地化搜索结果面板文案，导航栏搜索按钮与仓库图标仍带组件库的英文默认值（`Search`、`Source Code`）。两者由 `includes/footer.html` 的脚本按 zh 语言包同义中文补齐，且只在命中英文原文时替换，Quarto 自行本地化后脚本自动失效。新增界面脚本放在同一 include，不再叠加第二份。组件视觉（代码块、callout、侧栏 active、首页卡片）见本文件「组件约定」，不在此重复。
...
`1280/768/390` 的明暗两态和触屏复制按钮，并在窄屏展开左栏确认浮层底色不透明。需要刷新编译缓存时执行一次完整渲染。
