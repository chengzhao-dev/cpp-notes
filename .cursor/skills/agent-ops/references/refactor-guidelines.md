...
- 分工边界与「同一知识点只允许一个出处」的权威定义在 `.cursor/skills/catalog.md`「skill 与 knowledge 的分工」，其他文件只引用不复制。
...
- 完成后运行 `python .cursor/skills/agent-ops/scripts/run.py check`；涉及知识库再跑 `kb-index`、`kb-check` 和 `kb-eval`。
