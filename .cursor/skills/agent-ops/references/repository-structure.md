...
两类页面的组织依据是知识库 `quarto-docs` 域的两个条目，分别查询（多词短语会整体匹配不到，必须单 token）：`python .cursor/skills/agent-ops/scripts/run.py kb-search "块序列" --domain quarto-docs` 与 `python .cursor/skills/agent-ops/scripts/run.py kb-search "卡片" --domain quarto-docs`。
