...
def check_contracts(book_dir, htmls, css_pairs):
    ...
    results.append((
        ...
        ["每页 <head> 需 <link rel=\"icon\" … favicon.svg>；源文件见 .cursor/skills/quarto-theme/assets/theme/assets/favicon.svg，"
         "由 .cursor/skills/python-tools/scripts/maintenance/generate_favicon.py 生成"],
    ))
    ...
    source_css_text = "\n".join(
        path.read_text(encoding="utf-8", errors="ignore")
        for path in sorted(Path(__file__).resolve().parents[4].glob(".cursor/skills/quarto-theme/assets/theme/css/*.css"))
    )
    shared_selectors = (
    ...
    results.append((
        ...
        [
            ".cursor/skills/quarto-theme/assets/theme/css/code.css 应让 pre.sourceCode、pre:not(.sourceCode) 与 div.sourceCode 共用布局",
            "背景、边框、字体、字号、行高和内边距应引用 .cursor/skills/quarto-theme/assets/theme/css/tokens.css 的代码令牌",
        ],
    ))
