...
零新增元数据：READ 清单直接解析 tasks/**/<chapter>.md 里的「必读」行，
...
def parse_required_read(task_file):
    ...
    for line in task_file.read_text(encoding="utf-8").splitlines():
        ...
        for cand in TICK_RE.findall(line):
            ...
            if cand.startswith((".cursor/", "knowledge/", "content/")):
                paths.append(cand)
...
def find_chapter(target, root):
    ...
    task = root / ".cursor" / "skills" / "cpp-content" / "references" / "tasks" / part / f"{chapter}.md"
    if not qmd.is_file():
        qmd = None
...
DOMAIN_READ = {
    "theme": [".cursor/skills/quarto-theme/assets/theme/css/", ".cursor/skills/quarto-theme/SKILL.md",
              ".cursor/skills/quarto-theme/references/design-tokens.md"],
    "dev": [".cursor/skills/agent-ops/assets/config/editorconfig", ".cursor/skills/python-tools/scripts/scaffold/"],
    "repo": [".cursor/skills/agent-ops/references/repository-structure.md", ".cursor/skills/agent-ops/references/refactor-guidelines.md", "AGENTS.md"],
}
