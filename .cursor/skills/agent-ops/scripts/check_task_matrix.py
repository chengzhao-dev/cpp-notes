...
GEN = ROOT / ".cursor" / "skills" / "python-tools" / "scripts" / "maintenance" / "generate_tasks.py"
SCOPE = ROOT / ".cursor" / "skills" / "agent-ops" / "scripts" / "scope.py"
...
def paths_in(cell):
    ...
    for cand in PATH_RE.findall(cell):
        ...
        if cand.startswith(("content/", "code/", ".cursor/", "knowledge/")) and "build/" not in cand:
            out.append(cand)
    return out
