...
```powershell
python .cursor/tools/run.py kb-search "GitHub Pages 部署方式与产物分支" --domain tooling
```
...
2. `quarto render` → `python3 handbook/scripts/build/defer-mermaid.py`（本地 render 不自动跑，CI 必须执行）。
...
- [ ] 示例经 `.cursor/skills/cpp-content/scripts/verify_examples.py` 编译校验
