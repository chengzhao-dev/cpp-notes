...
- 步骤：checkout → setup Quarto → setup Python → `quarto render` → `.cursor/skills/python-tools/scripts/render/defer_mermaid.py` → `peaceiris/actions-gh-pages` 推 `_book/` 到 `gh-pages`（`force_orphan`）→ 可选校正 Pages source
...
1. **render-check.yml**（Pull Request 门禁）：
   ...
   - 执行阶段：
     ...
     - python3 .cursor/skills/python-tools/scripts/render/defer_mermaid.py：验证渲染后处理脚本。
...
- 当 CI 失败时，先在本地通过 .cursor/skills/agent-ops/scripts/run.py check 与 .cursor/skills/agent-ops/scripts/run.py verify 进行复现，禁止盲目推 commit 试错。
