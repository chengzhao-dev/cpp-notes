...
正文按“引言 → 前置条件 → 问题场景 → 心智模型 → 最小示例 → 实际操作 → 验证 → 常见错误 → 回顾”推进。章首不单列目标小节，读者收益写进引言段。前置条件与命令分开写，并列条件用无序列表，有先后关系的动作用有序列表。标题必须代表独立任务、概念、示例、验证或排错流程；短内容不单独创建 `###`。标题后的第一句直接兑现承诺，每段只引入一个新对象。
