...
用法：
    python .cursor/skills/python-tools/scripts/retriever.py "shared_ptr 循环引用怎么打破"
    python .cursor/skills/python-tools/scripts/retriever.py --explain "std::span 和裸指针加长度比有什么好处"
    python .cursor/skills/python-tools/scripts/retriever.py --domain cpp_core --level 5 --budget 2500 "所有权转移"
    python .cursor/skills/python-tools/scripts/retriever.py --rounds 2 --json "vector 为什么比 list 快"
...
def retrieve(query: str, domain: str = "", subdomain: str = "", level=None,
    ...
    if not kb.DB_PATH.is_file():
        raise SystemExit("FAIL  retriever  索引不存在，先运行 python .cursor/skills/python-tools/scripts/indexer.py")
    registry = kb.read_json(kb.REGISTRY_PATH, {"documents": {}, "chunks": {}})
    if not registry.get("chunks"):
        raise SystemExit("FAIL  retriever  注册表为空，先运行 python .cursor/skills/python-tools/scripts/indexer.py")
    ...
    if result.get("mode") == "toc":
        ...
        for row in result["parents"]:
            print("  L2 " + str(row["token_count"]).rjust(4) + "  [" + row["ref_id"] + "] "
                  + row["text"][:120])
        print("      取正文：python .cursor/skills/python-tools/scripts/retriever.py --parent <ref_id>")
        return 0
