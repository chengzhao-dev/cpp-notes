...
问题场景 → 心智模型 → 规则 → 最小示例 → 行为解释 → 正确/错误对照 → 限制与误区 → 验证 → FAQ（≤2–3 条）。
...
使用内置 `note`、`tip`、`warning`、`important` 或 `caution`，并保留全局中文类型标题。不要在块内重复写“最佳实践”“关键洞察”或“深入”；自定义 `.callout-*` 类会被 Quarto 静默丢弃，见 `.cursor/skills/quarto-docs/references/quarto/rendering-and-output.md` 陷阱 #12。
