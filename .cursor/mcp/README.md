...
服务通过 stdin 接收 JSON-RPC 2.0 消息，通过 stdout 返回 JSON-RPC 2.0 消息；日志写入 stderr。MCP 客户端通常会自动完成下面的初始化流程：
...
| `project_check` | 运行 `run.py check` | 可能生成检查缓存 |
...
Windows 如果 `python` 不在 PATH 中，把 `command` 换成本机 Python 3.12+ 的绝对路径；不要把个人路径提交到仓库配置。
