...
为什么需要它：3.1 的延迟指标只能在真实体量上验证。当前库只有百级 Chunk，
kb-check 的 P95 好看不代表十万级仍然好看，尤其向量路是全表扫描。
本脚本用真实知识文件派生合成 Chunk，按 1k/10k/50k/100k 逐级测量召回侧耗时，
给出可复现的曲线和「必须换 ANN 的拐点」，不改生产索引。

用法：
    python scripts/scale_benchmark.py                # 默认 1k/10k/50k/100k
    python scripts/scale_benchmark.py --sizes 1000,10000 --repeats 5
...
def real_paragraphs() -> list[str]:
    ...
    paras: list[str] = []
    for path in sorted(kb.DOMAINS_ROOT.rglob("*.md")):
        ...
        for block in re.split(NL + NL + "+", body):
            ...
            if len(block) >= 120:
                paras.append(block)
    if len(paras) < 40:
        raise SystemExit("FAIL  scale  真实段落不足，先确认 knowledge/domains 已入库")
    return paras
    ...
    while index < target:
        ...
        if doc_id not in documents:
            documents[doc_id] = {
                ...
                "domain": "cpp_core" if index % 2 else "tooling",
                "subdomain": "scale", "updated": "2026-09-08",
                "tags": ["pointer", "memory"], "levels": [0, 9],
                "branch": kb.DEFAULT_BRANCH, "source_hash": "",
            }
...
def main() -> int:
    ...
    print("规模基准  真实段落=" + str(len(paras)) + "  探针查询=" + str(len(probes))
          + "  重复=" + str(args.repeats) + "  上限 P95=" + str(int(P95_LIMIT_MS)) + "ms")
    failures: list[str] = []
    previous_rows: list[tuple[int, dict]] = []
    for target in sizes:
        documents, chunks = seed_chunks(target, paras)
        tmp = Path(tempfile.mkdtemp(prefix="kbscale-"))
        db_path = tmp / ("scale-%d.sqlite" % target)
        con = sqlite3.connect(db_path)
        indexer.connect(con)
        build_s = build(con, documents, chunks)
        stats = measure(con, probes, args.repeats)
        bytes_per_row = db_path.stat().st_size // max(1, len(chunks))
        print("  Chunk=" + str(len(chunks)).rjust(7)
              + "  索引=" + format(build_s, "6.2f") + "s"
              + "  BM25 P95=" + format(stats["bm25_p95"], "8.1f") + "ms"
              + "  向量 P95=" + format(stats["vector_p95"], "9.1f") + "ms"
              + "  双路合计 P95=" + format(stats["both_p95"], "9.1f") + "ms"
              + "  库=" + format(db_path.stat().st_size / 1048576.0, "7.1f") + "MB"
        ...
        if stats["both_p95"] >= P95_LIMIT_MS:
            failures.append("Chunk=" + str(len(chunks)) + " 双路 P95 "
                            + format(stats["both_p95"], ".0f") + "ms 超过 "
                            + str(int(P95_LIMIT_MS)) + "ms")
    if previous_rows:
    ...
    if failures:
        print("FAIL  scale  向量路全表扫描在下列体量已破 P95（需要 ANN/分片）")
        for row in failures:
            print("      " + row)
        return 1
    print("PASS  scale  全部级别双路 P95 低于 " + str(int(P95_LIMIT_MS)) + "ms")
    return 0
