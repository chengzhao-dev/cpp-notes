...
分支与版本（§3.6）：预过滤按 branch，supersedes 指向的旧版命中乘 0.35 惩罚，
  冲突表里较旧的一方按块重合度降权（最多降一半）。--all-branches 供体检/评测混排。
...
用法：
    python scripts/retriever.py "shared_ptr 循环引用怎么打破"
    python scripts/retriever.py --toc "指针学习路线图"     # 先看摘要地图
    python scripts/retriever.py --parent cpp-core-pointer-roadmap-v2__P__RAII-4ea9ceec
    python scripts/retriever.py --branch legacy-cpp98 "auto_ptr 为什么危险"
    python scripts/retriever.py --explain "std::span 和裸指针加长度比有什么好处"
    python scripts/retriever.py --domain cpp_core --level 5 --budget 2500 "所有权转移"
    python scripts/retriever.py --rounds 2 --json "vector 为什么比 list 快"
...
import kb_util as kb  # noqa: E402
...
def cache_key(query, domain, subdomain, level, tags, branch, budget, rounds):
    """缓存键含全部影响结果的参数：少带一个就会命中「看着对但其实是别的过滤条件」的旧结果。"""
    shape = chr(124).join([
        "q=" + query, "d=" + domain, "s=" + subdomain, "l=" + str(level),
        "t=" + ",".join(sorted(tags or [])), "b=" + branch,
...
def stale_factors(con: sqlite3.Connection, registry: dict, branch: str) -> dict:
    """返回 doc_id -> 惩罚系数（<1 才降权）。

    两个来源：frontmatter 的 supersedes（明确的版本替代，旧版一律降到
    SUPERSEDED_FACTOR），以及 conflict 表（同一分支内两份文档覆盖同一批概念且新版更新更晚，
    旧版按概念名集合的重合度降权，最多降一半）。

    只在「更新的那份确实落在本次检索分支里」时才惩罚旧版，否则会出现
    检索 legacy-cpp98 时把 C++98 正文降权、而 C++17 版根本不在结果里的荒谬行为。
    """
    factors: dict[str, float] = {}
    docs = registry["documents"]
    for doc_id, doc in docs.items():
        target = str(doc.get("supersedes") or "")
        if target and target != doc_id and doc.get("branch", kb.DEFAULT_BRANCH) == branch:
    ...
    for older, newer, overlap in rows:
        if older not in docs or newer not in docs:
            continue
        if docs[newer].get("branch", kb.DEFAULT_BRANCH) != branch:
            continue
...
def prefilter(con: sqlite3.Connection, registry: dict, domain: str, subdomain: str,
              level, tags: list[str], branch: str = kb.DEFAULT_BRANCH):
    """Stage 2：元数据预过滤（含分支），把候选缩到「可能被读到」的最小集合。

    分支必须在这里过滤，不能等排完序再丢：跨分支混排会让 main 的新知识把
    legacy 旧知识挤掉，也可能反过来；两种情况都拿不到可解释的结果。
    ...
    for doc in registry["documents"].values():
        if branch and doc.get("branch", kb.DEFAULT_BRANCH) != branch:
            continue
...
def vector_search(con: sqlite3.Connection, query: str, allow, limit: int) -> list[str]:
    probe = embed(query)
    scored = []
    for chunk_id, blob in con.execute(
        "SELECT chunk_id, vector FROM chunks WHERE kind='child' AND status='live' AND vector IS NOT NULL"
    ):
        if allow is not None and chunk_id not in allow:
            continue
        scored.append((cosine(probe, blob), chunk_id))
...
def retrieve(query: str, domain: str = "", subdomain: str = "", level=None,
             ...
             explain: bool = False, branch: str = kb.DEFAULT_BRANCH, toc: bool = False):
    """五阶段检索 + 分支/版本降权 + 滑动窗口 + 24h 查询缓存。toc=True 时只回摘要地图。"""
    started = time.time()
    if not kb.DB_PATH.is_file():
        raise SystemExit("FAIL  retriever  索引不存在，先运行 python scripts/indexer.py")
    registry = kb.read_json(kb.REGISTRY_PATH, {"documents": {}, "chunks": {}})
    if not registry.get("chunks"):
        raise SystemExit("FAIL  retriever  注册表为空，先运行 python scripts/indexer.py")
    con = sqlite3.connect(kb.DB_PATH)
    tags = tags or []

    key = cache_key(query, domain, subdomain, level, tags, branch, budget, rounds)
    ...
    if toc or intent["kind"] == "navigation":
        ...
        payload = {
            "query": query, "mode": "toc", "branch": branch,
...
def fetch_parent(parent_id: str, budget: int | None, branch: str = kb.DEFAULT_BRANCH) -> dict:
    ...
    doc = registry["documents"].get(chunk["doc_id"], {})
    if doc.get("branch", kb.DEFAULT_BRANCH) != branch:
        raise SystemExit("FAIL  retriever  " + parent_id + " 属于分支 "
                         + str(doc.get("branch")) + "，请加 --branch")
    controller = TokenBudgetController(budget)
    ...
    return {
        ...
        "branch": doc.get("branch", kb.DEFAULT_BRANCH), "content": chunk["content"],
    }
...
def main() -> int:
    ...
    parser.add_argument("--branch", default=kb.DEFAULT_BRANCH,
                        help="知识分支，默认 main；非 main 用 knowledge/branches/<name>/")
    parser.add_argument("--all-branches", action="store_true",
                        help="不按分支过滤（体检/评测用，会混排不同版本）")
    ...
    branch = "" if args.all_branches else args.branch
    if args.parent:
        row = fetch_parent(args.parent, args.budget, branch or kb.DEFAULT_BRANCH)
        if args.json:
            print(json.dumps(row, ensure_ascii=False, indent=1))
        else:
            print("----- " + row["heading_path"] + "  [parent, "
                  + str(row["token_count"]) + " 令牌, branch=" + row["branch"] + "] -----")
            print(row["content"])
        return 0
    if not args.query:
    ...
    result = retrieve(args.query, args.domain, args.subdomain, args.level, args.tag,
                      args.budget, min(max(args.rounds, 1), 3), args.explain,
                      branch, args.toc)
    if args.json:
    ...
    if result.get("mode") == "toc":
        print("查询 " + result["query"] + "  分支=" + (result["branch"] or "all")
              + "  模式=摘要地图  摘要令牌=" + str(result["budget"]["used"])
        ...
        for row in result["parents"]:
            print("  L2 " + str(row["token_count"]).rjust(4) + "  [" + row["ref_id"] + "] "
                  + row["text"][:120])
        print("      取正文：python scripts/retriever.py --parent <ref_id>")
        return 0

    print("查询 " + result["query"] + "  意图=" + result["intent"]["kind"]
          + "  分支=" + (result["branch"] or "all")
    ...
    if not result["injected"]:
        print("FAIL  retriever  无命中；检查 --domain/--level/--branch 是否过滤过窄，或先跑 indexer")
        return 2
    if args.explain:
