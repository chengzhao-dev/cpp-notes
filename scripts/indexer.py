...
三层索引的实际落地：
  ...
  Layer 2 向量：chunks.vector 存定长 packed-float 数组，检索时暴力扫余弦。向量由
               本文件的 embed() 生成，无外部模型时是 hashing trick 的词分布向量，
               不是神经嵌入；接真实模型只需替换 embed()。暴力扫的代价是 O(N)，
               十万级 Chunk 之后必须换成真正的 ANN（HNSW），届时也只替换 search 侧。
...
用法：
    python scripts/indexer.py              # 增量构建（先跑 chunker）
    python scripts/indexer.py --rebuild    # 推倒重建
    python scripts/indexer.py --verbose    # 打印每层的行数与耗时
...
def connect(con: sqlite3.Connection) -> None:
    ...
    # 旧库没有 branch 列时补列：增量必须能跑在上一版产物上
...
def upsert(con: sqlite3.Connection, chunk: dict, doc: dict, previous: dict) -> str:
    ...
    payload = (
        ...
        embed(chunk["content"]), branch,
    )
    columns = ("chunk_id, parent_id, doc_id, kind, heading_path, content, content_hash,"
               " token_count, domain, subdomain, updated, tags, levels, status, vector, branch")
    marks = ", ".join(["?"] * 16)
    con.execute(
...
def doc_concepts(registry: dict) -> dict:
    ...
    `.cursor/skills/quarto-docs/references/…` 贡献 `references`、`GitHub Pages`
...
def detect_conflicts(con: sqlite3.Connection, registry: dict) -> int:
    """同一分支内两份文档覆盖同一批概念、且新版更新更晚 → 记一条待确认冲突。

    三个限定条件都是实测逼出来的，缺一个就会误报或误降权：
    1. 重合度取文档级概念名集合的 Jaccard。旧实现按 chunk_id 取交集，而 chunk_id
       以 doc_id 为前缀，跨文档交集恒空 → overlap 恒 0 → 检索侧降权从未生效。
    2. 只比同一 branch：legacy-cpp98 与 main 对「智能指针」的描述本来就该不同，
       那是 §3.6 有意承载的分支分歧，不是冲突。
    3. 跳过已被 supersedes 关联的一对：版本演进已有明确结论，再报冲突是重复告警。
    ...
    branch_of = {d: str(docs[d].get("branch", kb.DEFAULT_BRANCH)) for d in docs}
    ...
    for index, first in enumerate(ids):
        for second in ids[index + 1:]:
            ...
            if not concepts.get(older) or not concepts.get(newer):
                continue
            if branch_of[older] != branch_of[newer]:
                continue
...
def main() -> int:
    ...
    if concepts == 0 or live == 0:
        print("FAIL  indexer  索引为空，检查 knowledge/domains 是否有合规文件")
        con.close()
        return 1
