...
用法：
    python scripts/health_check.py            # 体检（有告警时退出码 1）
    python scripts/health_check.py --gate     # 给 run.py check 用：只在 FAIL 时非 0
    python scripts/health_check.py --verbose   # 打印每条超阈值项的具体位置
...
def check_branches(registry: dict, con) -> tuple[dict, list[str], int]:
    """按分支统计文档/Chunk，检查 supersedes 是否指向存在的文档，并数出冲突候选。

    悬空的 supersedes 是真实的腐化信号：目标文档改名或删掉之后，旧版永远拿不到
    降权，检索会把两版并列返回。这项断言廉价且可修（改 frontmatter 或删字段）。

    冲突候选不是错误：设计上只做"同一分支内两份文档覆盖同批概念且 updated 更新晚于对方"的
    ...
    for doc in registry.get("documents", {}).values():
        ...
        stats = per_branch.setdefault(branch, {"documents": 0, "chunks": 0})
        stats["documents"] += 1
    ...
    for branch, chunks in con.execute(
        "SELECT branch, count(*) FROM chunks WHERE status='live' GROUP BY branch"
    ):
        key = branch or kb.DEFAULT_BRANCH
        per_branch.setdefault(key, {"documents": 0, "chunks": 0})["chunks"] = chunks
    conflicts = con.execute("SELECT count(*) FROM conflict").fetchone()[0]
    return per_branch, problems, conflicts
...
def measure_latency(samples: int) -> tuple[float, float]:
    ...
    queries = [
        ...
        "指针的学习顺序和路线图", "void* 类型擦除", "内存对齐 alignas", "weak_ptr lock 用法",
...
def run(gate: bool, verbose: bool, samples: int) -> int:
    ...
    branches, branch_problems, conflicts = ({}, [], 0)
    if holder and kb.DB_PATH.is_file():
        probe = sqlite3.connect(kb.DB_PATH)
        branches, branch_problems, conflicts = check_branches(holder[0], probe)
        probe.close()
    counts["dangling"] = len(branch_problems)
    problems = problems + branch_problems
