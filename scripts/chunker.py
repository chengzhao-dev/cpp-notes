#!/usr/bin/env python3
"""语义分块器：把 knowledge/domains 下的 Markdown 切成 Parent / Child 两层 Chunk。
...
用法：
    python scripts/chunker.py               # 增量：只重切变更文件
    python scripts/chunker.py --all         # 忽略哈希，强制全量重切
    python scripts/chunker.py --verbose      # 逐篇打印 Parent/Child 数量
    python scripts/chunker.py --show         # 打印每篇的 Child 标题路径与令牌数
